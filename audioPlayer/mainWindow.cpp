...
MainWindow::MainWindow(QWidget* parent)
    ...
    entireLayout->addWidget(centralWidget);
    entireLayout->setSpacing(0);
    //entireLayout->addLayout(mainLayout);
    ...
    repeatMode = 0;
    removeTracksSelectionMode = false;
    removePlaylistSelectionMode = false;
    tracksDragging = false;
    showPlaylistsFlag = false;
    isTrackDownloading = false;

    centralWidget->setStyleSheet(backgroundColorStyle);

    topLayout->setAlignment(Qt::AlignBottom);
    settingsButtonLayout->setAlignment(Qt::AlignCenter);
    settingsButton->setFixedSize(15, 15);
    connect(settingsButton, &QPushButton::clicked, this, &MainWindow::openSettingsWindow);
    downloadTrackButton->setFixedSize(15, 15);
    downloadTrackButton->setIconSize(QSize(15, 15));
    ...
    connect(moveTrackButton, &QPushButton::clicked, this, &MainWindow::moveTracks);
...
void MainWindow::updateStyles() {
    ...
    for (PlaylistButton* playlist : *playlistList) {
        for (MusicButton* musicButton : *playlistList->at(playlist->getPlaylistNumber() - 1)->getList()) {
            if (QUrl::fromLocalFile(musicButton->getTrackPath()) == player->source()) {
                musicButton->setStyleSheet(musicButtonClickedStyle);
            }
            else {
                musicButton->setStyleSheet(musicButtonStyle);
            }
        }
    }
...
void MainWindow::updateMusicButtonIndices() {
    for (int i = 0; i < tracksScrollAreaLayout->count(); ++i) {
        MusicButton* button = dynamic_cast<MusicButton*>(tracksScrollAreaLayout->itemAt(i)->widget());
        if (button) {
            button->setTrackNumber(i + 1);
        }
    }
}
...
void MainWindow::resizeEvent(QResizeEvent* event) {
    ...
    qDebug() << titleBarColorStyle;
    if (playlistList && playlistList != nullptr)
        qDebug() << playlistList->at(currentPlaylistNumber)->getList()->last()->getTrackPath();
}
